...
NS_LOG_COMPONENT_DEFINE ("OvnisWifiHelper");
...
OvnisWifiChannelHelper::OvnisWifiChannelHelper ()
{}

OvnisWifiChannelHelper
OvnisWifiChannelHelper::Default (void)
{
  OvnisWifiChannelHelper helper;
...
void 
OvnisWifiChannelHelper::AddPropagationLoss (std::string type,
...
void 
OvnisWifiChannelHelper::SetPropagationDelay (std::string type,
					    std::string n0, const AttributeValue &v0,
					    std::string n1, const AttributeValue &v1,
					    std::string n2, const AttributeValue &v2,
					    std::string n3, const AttributeValue &v3,
...
Ptr<OvnisWifiChannel>
OvnisWifiChannelHelper::Create (void) const
{
  Ptr<OvnisWifiChannel> channel = CreateObject<OvnisWifiChannel> ();
...
OvnisWifiPhyHelper::OvnisWifiPhyHelper ()
  : m_channel (0),
    m_pcapDlt(PcapHelper::DLT_IEEE802_11)
{
  m_phy.SetTypeId ("ns3::OvnisWifiPhy");
}

OvnisWifiPhyHelper
OvnisWifiPhyHelper::Default (void)
{
  NS_LOG_FUNCTION_NOARGS();

  OvnisWifiPhyHelper helper;

  //  !!!!!!! Should not exist  !!!!!!!
  Ptr<Object> o = CreateObject<OvnisErrorRateModel>();

  helper.SetErrorRateModel ("ns3::OvnisErrorRateModel");
  return helper;
}

void 
OvnisWifiPhyHelper::SetChannel (Ptr<OvnisWifiChannel> channel)
{
  m_channel = channel;
}
void 
OvnisWifiPhyHelper::SetChannel (std::string channelName)
{
  Ptr<OvnisWifiChannel> channel = Names::Find<OvnisWifiChannel> (channelName);
  m_channel = channel;
}
void 
OvnisWifiPhyHelper::Set (std::string name, const AttributeValue &v)
{
  m_phy.Set (name, v);
}

void 
OvnisWifiPhyHelper::SetErrorRateModel (std::string name,
                                     std::string n0, const AttributeValue &v0,
                                     std::string n1, const AttributeValue &v1,
                                     std::string n2, const AttributeValue &v2,
                                     std::string n3, const AttributeValue &v3,
...
Ptr<WifiPhy> 
OvnisWifiPhyHelper::Create (Ptr<Node> node, Ptr<WifiNetDevice> device) const
{
  Ptr<OvnisWifiPhy> phy = m_phy.Create<OvnisWifiPhy> ();
...
void 
OvnisWifiPhyHelper::SetPcapDataLinkType (enum SupportedPcapDataLinkTypes dlt)
{
  switch (dlt)
    {
    case DLT_IEEE802_11:
    ...
    case DLT_IEEE802_11_RADIO:
      ...
      return;
    default:
      NS_ABORT_MSG ("OvnisWifiPhyHelper::SetPcapFormat(): Unexpected format");
    }
}
...
void 
OvnisWifiPhyHelper::EnablePcapInternal (std::string prefix, Ptr<NetDevice> nd, bool promiscuous, bool explicitFilename)
{
  //
  ...
  if (device == 0)
    {
      NS_LOG_INFO ("OvnisWifiHelper::EnablePcapInternal(): Device " << &device << " not of type ns3::WifiNetDevice");
      return;
    }

  Ptr<WifiPhy> phy = device->GetPhy ();
  NS_ABORT_MSG_IF (phy == 0, "OvnisWifiPhyHelper::EnablePcapInternal(): Phy layer in WifiNetDevice must be set");
...
void 
OvnisWifiPhyHelper::EnableAsciiInternal (
  Ptr<OutputStreamWrapper> stream, 
  std::string prefix, 
  Ptr<NetDevice> nd,
  bool explicitFilename)
{
  ...
  if (device == 0)
    {
      NS_LOG_INFO ("OvnisWifiHelper::EnableAsciiInternal(): Device " << device << " not of type ns3::WifiNetDevice");
      return;
    }
